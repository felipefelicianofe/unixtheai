...
const isNeutralEntry = (h: HistoryRow) => h.status === "NEUTRAL" || h.signal === "NEUTRO" || h.signal === "NEUTRAL";
...
const AutoGerenciamento = () => {
  ...
  // Build a map of asset → live price from the monitoring engine (updates every 10s)
  ...
  const { data: configs, isLoading: loadingConfigs } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error("ERRO CONFIGS SUPABASE:", error);
        toast.error("Erro Configs: " + error.message);
        throw error;
      }
  ...
  const { data: history, isLoading: loadingHistory } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .limit(10000);
      if (error) {
        console.error("ERRO HISTORY SUPABASE:", error);
        toast.error("Erro History: " + error.message);
        throw error;
      }
  ...
  const { data: refinementWeights, isLoading: loadingWeights } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('management_refinement_weights' as never)
        .select('*')
        .order('win_rate', { ascending: false });
      if (error) throw error;
      return data as RefinementWeight[];
    }
  });
  ...
  const { data: refinementLogs } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('management_refinement_log' as never)
        .select('*')
        .order('created_at', { ascending: false })
        .limit(20);
      if (error) throw error;
      return data as RefinementLog[];
    }
  });
  ...
  const verifyMutation = useMutation({
    ...
    onError: (err: Error) => toast.error("Erro na verificação: " + err.message),
  });
  ...
  const refineMutation = useMutation({
    ...
    onError: (err: Error) => toast.error("Erro no refinamento: " + err.message),
  });
  ...
  const createConfigMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setNewAsset("");
      setNewTimeframe("");
      // Refresh monitoring engine configs
      refreshConfigs();
      try {
  ...
  // ── DEDUP: Remove duplicate active entries per asset from display ──
  // For PENDING/WIN_TP1/WIN_TP2 non-neutral entries, keep only the most recent per asset
  const dedupedHistory = (() => {
    const seenActiveAssets = new Set<string>();
    return filteredHistory.filter((h) => {
      const isActive = isActiveEntry(h);
      const isNeutral = isNeutralEntry(h);
      
      if (isActive && !isNeutral) {
        // Only keep the first (most recent, since history is sorted desc by created_at)
        if (seenActiveAssets.has(h.asset)) {
  ...
  const displayedHistory = showNeutros
    ? dedupedHistory
    : dedupedHistory.filter((h) => !(h.status === 'NEUTRAL' || ((h.signal === 'NEUTRO' || h.signal === 'NEUTRAL') && h.status === 'PENDING')));
  ...
  const isActiveEntry = (h: HistoryRow) => ['PENDING', 'WIN_TP1', 'WIN_TP2'].includes(h.status) && !h.closed_at;
  ...
  const totalPending = filteredHistory.filter((h) => isActiveEntry(h) && !isNeutralEntry(h)).length;

  // Stats: only finalized actionable entries (WIN_TP3 + LOSS + TP-closed-at-BE)
  // Note: WIN_TP1/WIN_TP2 are ONLY finalized if they have a closed_at (hit BE)
  const totalFinalized = actionableHistory.filter((h) => 
    h.status === 'WIN_TP3' || h.status === 'WIN' || h.status === 'LOSS' || 
    (['WIN_TP1', 'WIN_TP2'].includes(h.status) && h.closed_at)
  ).length;
  
  const totalWins = actionableHistory.filter((h) => 
    h.status === 'WIN_TP3' || h.status === 'WIN' || 
    (['WIN_TP1', 'WIN_TP2'].includes(h.status) && h.closed_at)
  ).length;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="relative z-10 pt-24 pb-24 px-4 max-w-7xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold text-foreground">Auto Gerenciamento</h1>
          <p className="text-muted-foreground mt-2">
            Monitoramento 24/7 com sinais de entrada em tempo real.
          </p>
        </motion.div>

        {/* Dashboard Indicators — TOP */}
        <DashboardMetrics
          winRate={winRate}
          totalWins={totalWins}
          totalLoss={totalLoss}
        ...
        {/* Active Signals Section — only entry signals */}
              ...
              {[...activeSignals]
                ...
                .map((signal) => (
                  <ActiveSignalCard
                    key={signal.asset}
                    state={signal}
                    onManualClose={async (asset) => {
                      await manualClose(asset);
                      queryClient.invalidateQueries({ queryKey: ['auto-management-history'] });
                    }}
                  />
                ))}
        ...
        <Card className="glass-panel border-muted bg-background/50">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              {Array.from(new Set(history?.map((h: HistoryRow) => h.asset) || [])).map(asset => {
                const assetStr = asset as string;
                const assetAll = history?.filter((h: HistoryRow) => h.asset === asset) || [];
                const assetActionable = assetAll.filter((h: HistoryRow) => !isNeutralEntry(h));
                const assetWins = assetActionable.filter((h: HistoryRow) => h.status?.startsWith('WIN')).length;
                const assetLosses = assetActionable.filter((h: HistoryRow) => h.status === 'LOSS').length;
                const assetNeutrals = assetAll.filter((h: HistoryRow) => isNeutralEntry(h)).length;
                const totalFinished = assetWins + assetLosses;
                const assetWinRate = totalFinished > 0 ? (assetWins / totalFinished) * 100 : 0;
                const assetUL = assetActionable.reduce((sum: number, h: HistoryRow) => {
                  if (h.status === 'WIN_TP1') return sum + 1;
                  if (h.status === 'WIN_TP2') return sum + 2;
                  if (h.status === 'WIN_TP3' || h.status === 'WIN') return sum + 3;
                  if (h.status === 'LOSS') return sum - 1;
              ...
              <CardContent>
                <div className="space-y-3">
                  {loadingHistory ? (
                    ...
                    <>
                    {displayedHistory.slice(0, historyLimit).map((h: HistoryRow) => {
                      ...
                      // Use live price from monitoring engine (10s) if available, else fallback to DB
                      const livePrice = h.asset ? livePriceMap.get(h.asset) : null;
                      const displayPrice = safeNum(livePrice ?? h.current_price);
                      // Recalculate P&L with live price when possible
                      let pnl: number | null = null;
                      let rawPnl: number | null = null;
                      if (displayPrice && entryPrice && !isNeutral) {
                        const dir = isLong ? 1 : -1;
                        rawPnl = ((displayPrice - entryPrice) / entryPrice) * dir * 100;
                        pnl = rawPnl * leverage;
                      } else {
                        rawPnl = safeNum(h.virtual_pnl_pct);
                        pnl = rawPnl !== null ? rawPnl * leverage : null;
                      }
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedRowId(isExpanded ? null : h.id)}>
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              {!isNeutral && displayPrice && isPending && (
                                <div className="text-right">
                                  <div className="text-sm font-semibold">${formatPrice(displayPrice)}</div>
                                  <div className="text-[10px] text-muted-foreground">
                                    {livePrice ? 'Preço Live' : 'Preço Atual'}
                                  </div>
                                </div>
                              )}
                          ...
                          {isExpanded && h.full_result && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="mt-4 pt-4 border-t border-border/50">
                              <AnalysisResults data={typeof h.full_result === 'string' ? (() => { try { return JSON.parse(h.full_result); } catch { return h.full_result; } })() : h.full_result} onNewAnalysis={() => setExpandedRowId(null)} />
                            </motion.div>
                          )}
                        </motion.div>
                      );
                    ...
                    {displayedHistory.length > historyLimit && (
                      <div className="text-center pt-4">
                        <Button variant="outline" onClick={() => setHistoryLimit(prev => prev + 50)}>
                          Carregar mais ({displayedHistory.length - historyLimit} restantes)
                        </Button>
                      </div>
                    )}
                    </>
          ...
          <TabsContent value="refinement" className="space-y-6">
                ...
                <Card className="glass-panel border-green-500/20 bg-background/50">
                  <CardContent className="pt-4">
                    ...
                    <div className="text-2xl font-bold text-green-500">
                      {refinementWeights.filter((w: RefinementWeight) => w.trend === 'IMPROVING').length}
                    </div>
                  </CardContent>
                </Card>
                <Card className="glass-panel border-red-500/20 bg-background/50">
                  <CardContent className="pt-4">
                    ...
                    <div className="text-2xl font-bold text-red-500">
                      {refinementWeights.filter((w: RefinementWeight) => w.trend === 'DEGRADING').length}
                    </div>
                  </CardContent>
                </Card>
                <Card className="glass-panel border-muted bg-background/50">
                  <CardContent className="pt-4">
                    ...
                    <div className="text-2xl font-bold text-foreground">
                      {(refinementWeights.reduce((sum: number, w: RefinementWeight) => sum + (w.win_rate || 0), 0) / refinementWeights.length).toFixed(1)}%
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            <Card className="glass-panel border-muted bg-background/50">
              <CardHeader>
                ...
                <CardDescription>
                  Performance de cada indicador com base nos resultados reais do Auto Gerenciamento.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loadingWeights ? (
                  ...
                  const groups = new Map<string, RefinementWeight[]>();
                  refinementWeights.forEach((w: RefinementWeight) => {
                  ...
                  return (
                    <Accordion type="multiple" className="w-full space-y-2">
                      {Array.from(groups.entries()).map(([groupKey, items]) => {
                        const avgWr = items.reduce((s: number, w: RefinementWeight) => s + (w.win_rate || 0), 0) / items.length;
                        const improving = items.filter((w: RefinementWeight) => w.trend === 'IMPROVING').length;
                        const degrading = items.filter((w: RefinementWeight) => w.trend === 'DEGRADING').length;
            ...
            {refinementLogs && refinementLogs.length > 0 && (
              <Card className="glass-panel border-primary/20 bg-background/50">
                ...
                <CardContent className="space-y-4">
                  {(() => {
                    const latestMap = new Map<string, RefinementLog>();
                    (refinementLogs || []).forEach((log: RefinementLog) => {
                    ...
                    const hasBacktestData = latestLogs.some((l: RefinementLog) => l.projected_wr_new_weights !== null && l.projected_wr_new_weights !== undefined);
                    ...
                    return (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {latestLogs.filter((l: RefinementLog) => l.projected_wr_new_weights !== null).map((log: RefinementLog) => {
            ...
            {refinementLogs && refinementLogs.length > 0 && (
              <Card className="glass-panel border-muted bg-background/50">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {refinementLogs.map((log: RefinementLog) => {
            ...
            <Card className="glass-panel border-muted bg-background/50">
              ...
              <CardContent>
                {loadingDeleted ? (
                  ...
                  <div className="space-y-2">
                    {deletedHistory.map((h: HistoryRow) => {
                      const isNeutral = isNeutralEntry(h);
                      return (
        ...
        <Dialog open={!!backtestDetailLog} onOpenChange={(open) => !open && setBacktestDetailLog(null)}>
          <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto bg-background border-border">
            ...
            {(() => {
              ...
              const details = backtestDetailLog.backtest_details;
...
export default AutoGerenciamento;
